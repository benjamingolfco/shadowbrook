...
Integration tests in `Teeforce.Api.IntegrationTests` fail universally in this local environment with `System.InvalidOperationException: The server has not been started or no web application was configured`. This is a pre-existing infrastructure issue affecting all branches including main — it is not caused by code changes.
...
**How to apply:** When the verify-build hook fails and ALL failures are `The server has not been started or no web application was configured` across `Teeforce.Api.IntegrationTests`, this is the pre-existing local environment issue. Confirm that unit tests (Teeforce.Domain.Tests and Teeforce.Api.Tests) pass — if they do, the implementation is correct and the PR should be submitted. Integration tests run in CI (GitHub Actions). Do not attempt to fix this, modify tests, or look for the root cause in the code changes.
